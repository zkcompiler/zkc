// Two reduction bodies genuinely interleaved in the spine: A's first message,
// B's first, A's second, B's second, then the two challenges.  kernel.md
// section 1.4 admits this shape. Section 9.3 requires security pricing to use
// one exact composite rule or refuse (see interleaving-pin.test).
pir.protocol "interleaved" kappa {codecs = {scalar = "ts_be8"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  %t2, %mb0 = pir.slot %t1 "mb0" : "scalar" in "opb" as "m"
  %t3, %ma1 = pir.slot %t2 "ma1" : "scalar" in "opa" as "m" idx 1
