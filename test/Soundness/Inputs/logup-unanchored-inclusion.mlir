// A lookup whose consumed inclusion is about material the round never saw.
//
// Driven by logup-range-check.test. The three digests appear nowhere else in
// the artifact, so the entry would read "from inclusion about these false to
// identity over the round's columns true" — two statements about different
// objects, between which the bound relates nothing.
pir.protocol "logup_range_check" kappa {codecs = {scalar = "ts_be8"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  %inclusion = pir.instantiate "inclusion" anchors {multiplicities = "sha256:00ff000000000000000000000000000000000000000000000000000000000001", queries = "sha256:00ff000000000000000000000000000000000000000000000000000000000002", table = "sha256:00ff000000000000000000000000000000000000000000000000000000000003"} : !pir.claim<"logup_inclusion">
  %t0 = pir.begin
