// REQUIRES: uv
// The v4 same-point batch-opening shape.  Input descriptors are canonically
// ordered before the members digest and verifier operand vectors are formed;
// this is the exact independent-oracle twin for the batch transform target.
// RUN: zkc-opt %pir-seal-full %s -o %t.sealed
// RUN: zkc-translate --canonical %t.sealed -o %t.zkc
// RUN: %uv python -m oracle.parity encode kzg-batch > %t.oracle
// RUN: diff %t.zkc %t.oracle
// RUN: zkc-translate --id %t.sealed -o %t.zkc-id
// RUN: %uv python -m oracle.parity id kzg-batch > %t.oracle-id
// RUN: diff %t.zkc-id %t.oracle-id

pir.protocol "kzg_after" kappa {codecs = {fr = "fr_be32", g1 = "bls_g1_be48"}, iv = "artifact-id", sponge = "toy_duplex"} {
  ...
  pir.discharge %batch : !pir.claim<"batch_opening"> rule "zkc.terminal.kzg-batch-opening" checks {opening = "batch_ok"}
}
