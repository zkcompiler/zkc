...
pir.protocol "logup_range_check" kappa {codecs = {scalar = "ts_be8"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  // Each column's anchor is tied to the transcript value that carries it, so
  // the claim's identity rests on material the verifier saw rather than on
  // the reduction's word.
