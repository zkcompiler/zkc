// REQUIRES: uv
// Two independently discharged KZG openings: the input surface for
// generic same-point batching.  Closure is exact: each claim carries its
// value anchor, each semantic anchor is tied to verifier material, and the
// discharge cites the admitted KZG terminal rule.
// RUN: zkc-opt %pir-seal-full %s -o %t.sealed
// RUN: zkc-translate --canonical %t.sealed -o %t.zkc
// RUN: %uv python -m oracle.parity encode kzg-single > %t.oracle
// RUN: diff %t.zkc %t.oracle
// RUN: zkc-translate --id %t.sealed -o %t.zkc-id
// RUN: %uv python -m oracle.parity id kzg-single > %t.oracle-id
