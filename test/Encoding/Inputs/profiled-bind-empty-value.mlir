// A profiled seal-stage binding whose absorbed value is empty.
//
// Driven by value-profile-resolution.mlir. The value is the material
// reference, so an empty one would leave the transcript absorbing nothing in
// the table's place while a material binding grounded the anchor elsewhere.
pir.protocol "logup_range_check" kappa {codecs = {scalar = "ts_be8"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  %t1, %table = pir.bind %t0 "table" : profile "logup_table" stage seal = "" in "bus" as "table"
