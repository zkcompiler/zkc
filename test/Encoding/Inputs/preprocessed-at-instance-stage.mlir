// A preprocessed profile on an instance-stage binding.
//
// Driven by value-profile-resolution.mlir. Preprocessed content is fixed
// before any statement; an instance-stage binding's value arrives per
// statement and is absent at seal, so the anchor preimage the arity
// hypothesis is discharged at does not exist in the transcript.
pir.protocol "logup_range_check" kappa {codecs = {scalar = "ts_be8"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  %t1, %table = pir.bind %t0 "table" : profile "logup_table" stage instance in "bus" as "table"
  ...
  pir.material_bind %table to "sha256:3f2a1c8d5e7b9046a2c1e8f4d6b0937518a4c2e0f9d7b5638a1c4e2f0d9b7563" : !pir.val<profile "logup_table">
