// The artifact driver emits only canonical bytecode and the loader recomputes the
// canonical identity before returning the decoded carrier.
// RUN: rm -rf %t.dir
// RUN: zkc-seal %s %zkc-seal-full -o %t.dir | FileCheck --check-prefix=WROTE %s
// RUN: zkc-artifact %t.dir/*.mlirbc | FileCheck --check-prefix=LOADED %s
// WROTE: {{[0-9a-f]+}}.mlirbc
// LOADED: decoded artifact {{[0-9a-f]+}}
// LOADED: pir.sealed "relation_direct"
// LOADED: contract "zkc.check.relation-predicate"
// LOADED: rule "zkc.terminal.relation-direct"
