// The three-layer GKR chain with one ambient squeeze inside the composed
// span.  Everything else is the shipped fixture verbatim.
//
...
pir.protocol "gkr_width2_depth3" kappa {codecs = {scalar = "ts_be8"}, constants = {one = {class = "scalar", value = "1"}}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  // One squeeze that belongs to no reduction, between the second and
  // third layers.  It sits inside the span a three-layer composition
  // covers, and the composed error function does not index it.
  %t33a, %ambient = pir.chal %t33 "ambient" : "scalar" domain "gkr.ambient" space "2305843009213693951"
  %t34, %l2_rho = pir.chal %t33a "l2_rho" : "scalar" domain "gkr.layer2.rho" space "2305843009213693951"
