// Vulnerable ordering witness: alpha is sampled before the opening values
// that the ordered RLC contract requires in its prefix.
pir.protocol "sp1_rlc_vulnerable" kappa {codecs = {ext_field = "plonky3_bb31_ext4_tuple", fr = "fr_be32"}, iv = "artifact-id", sponge = "toy_duplex"} policy "analysis_only_artifact" {
  ...
  %t0 = pir.begin
  %t1, %alpha = pir.chal %t0 "alpha" : "ext_field" domain "sp1.opening-value-rlc.alpha" space "16428751811598850197311699254593454081"
  %t2, %v0 = pir.slot %t1 "value0" : "ext_field" in "opening_rlc" as "values"
  %t3, %v1 = pir.slot %t2 "value1" : "ext_field" in "opening_rlc" as "values" idx 1
  %t4, %v2 = pir.slot %t3 "value2" : "ext_field" in "opening_rlc" as "values" idx 2
  pir.end %t4
