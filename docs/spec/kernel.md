...
is a morphism of the free **premonoidal** category generated by
...
structure is what §9's budget arithmetic walks.

The category is premonoidal rather than symmetric monoidal, and the
difference is not a technicality. Interchange fails: a transformer's
body is a set of spine events, two bodies may interleave, and BIND
depends on which absorbed events precede which challenge — so two
transformers that touch the transcript do not commute, and reordering
them can change a verdict. What does commute is the **centre**: a
transformer whose body contains no absorbing event and no challenge
event neither writes what the transcript reads nor reads what it
writes, and those form a genuine symmetric monoidal sub-category. That
distinction is why §9.3 must be conservative about interleaved groups,
and it is also the shape of the condition under which it need not be:
an interleaved group decomposes per-transformer exactly when all but
one of its members is central. J2 is LIN: "no claim
