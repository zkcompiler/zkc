...
| Lookup arguments |  | ● |  | A logarithmic-derivative bus over committed columns, priced from what those commitments say they hold: the bound grows in the table arity, the arity is a fact of the value's profile rather than a producer's annotation, and a machine condition ties it to the opening challenge's space so an understated one refuses. The opening round is a single index into committed content, priced at what a spot check is worth without a low-degree test; a production lookup composes this bus with polynomial machinery that is not modelled here. The characteristic hypothesis of the cited lemma is checked, and honest multiplicities are carried as an obligation. |
