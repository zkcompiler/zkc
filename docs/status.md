...
| Lookup arguments | [`test/Encoding/logup-bus.mlir`](../test/Encoding/logup-bus.mlir), [`test/Soundness/logup-bus.test`](../test/Soundness/logup-bus.test), [`test/Encoding/value-profile-resolution.mlir`](../test/Encoding/value-profile-resolution.mlir) |
...
| Lookup arguments |  | ● |  | A logarithmic-derivative bus over committed columns, priced from what those commitments say they hold: the bound grows in the table arity, and the arity is a fact of the value's profile rather than a producer's annotation. A machine condition requires that declared content and the opening challenge's range agree, so an inconsistent understatement refuses; both are authored, so what stays ungrounded is the declaration against the realized commitment. Nothing in a transcript can ground it: a prover who committed to more than the profile declares answers every challenge in the declared range, and understatement is the direction that shrinks the bound. The judgment therefore carries it as an obligation, discharged at the construction the profile's binding route names, whose opening relation is what makes a declared length either true or infeasible to fake. The opening round is therefore priced at zero — it establishes nothing — and the claim leaves as a residual. One arity answers both counts here, so a lookup whose table and query column differ in length is not expressible. The cited lemma's characteristic hypothesis is checked against a field order the caller states and two hypotheses carry; honest multiplicities are carried as an obligation. |
