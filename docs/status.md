...
| Lookup arguments | [`test/Encoding/logup-range-check.mlir`](../test/Encoding/logup-range-check.mlir), [`test/Soundness/logup-range-check.test`](../test/Soundness/logup-range-check.test), [`test/Encoding/logup-bus.mlir`](../test/Encoding/logup-bus.mlir), [`test/Soundness/logup-bus.test`](../test/Soundness/logup-bus.test), [`test/Encoding/value-profile-resolution.mlir`](../test/Encoding/value-profile-resolution.mlir) |
...
| Lookup arguments |  | ● |  | A logarithmic-derivative lookup over committed columns, priced from what those commitments say they hold. The bound grows in the sizes of both sides and the two are read separately, by message role, so the canonical shape — a small table with many queries — states its own two numbers; the flagship is a range check whose table the statement fixes and which therefore enters as a public binding, the seat a preprocessed origin requires. The reduction consumes the inclusion and produces the identity at the sampled point, which is the passage a single challenge buys: soundness is priced at (table + lookups - 1)/space, one degree tight because every term of the cleared identity omits a factor, and completeness at (table + lookups)/space, the vanishing set counted as a set. The lemma's characteristic hypothesis is checked against a field order the caller states and two hypotheses carry, on the larger of the two sides, which is what the unequal case reduces to. Where the artifact fixes a relation between declared arities — the multiplicity column is indexed by the table — a condition checks it. Where it fixes none, the declared content of a lone commitment is carried as an obligation rather than checked, because a prover who committed to more answers every challenge honestly and no transcript arrangement separates the two; it is discharged at the binding route or the anchor preimage the profile names. Discharging the produced identity — helper columns, a running sum, and the quotient machinery that checks them — is not stated here and leaves as a residual. Because that discharge owns every check, these artifacts carry none, and projection refuses them rather than emitting a verifier that would accept any proof: this family has an analysis but no endpoint until the discharge lands — which is a fact about where this reduction's checks live rather than about reductions, since sumcheck routes a residual and projects. The evidence for it is therefore two internal legs, pricing and encoding, and two named absences: no emitted endpoint to exercise, and no external yardstick on the pricing (the in-repo re-derivation is the only independent reading; soundcalc's scope does not reach lookups). |
...
| Soundness analysis |  | ● |  | Selected conditional, notion-indexed derivations from admitted PIR, including the Fiat-Shamir compilation of an analysis-only family artifact end to end at its toy configuration: the interactive row, the grinding row that scales it, state restoration, and the duplex hop, whose bound carries each squeeze's exact sampling bias and the collision game the transcript's relation anchors scale. The value-faithful instance does not reach this path, for a reason upstream of it. |
