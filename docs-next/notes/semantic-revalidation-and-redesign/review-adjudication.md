...
| DOC-1 | Durable Realization semantics depend on temporary notes | `Confirmed` | This was confirmed at R1. Bounded K3-D subsequently removed `BelowOirPlanBasis` and `CheckedBelowOirPlanPlacement` from the durable base path: reachable Plan semantics enters the prover OIR quotient and Realization consumes OIR requirements only. A future Plan extension needs its own durable checked-placement contract. The original durable-to-note dependency is therefore repaired in the target, while full Stage 4B Realization design remains inactive. |
...
| REG-3 | `artifact_verify` has no target semantic home | `Confirmed` | This was confirmed for the R1 target. K2 and K3-D now split the boundary: a legacy imported-verification carrier without a supported K2 module-effect contract stops at PIR admission, while an already-admitted module effect without an OIR rule is projection-`Unsupported`; neither yields a partial target. Positive recursive or imported-verification semantics remain open and require an exact registered module contract rather than revival of the bundled legacy row. |
