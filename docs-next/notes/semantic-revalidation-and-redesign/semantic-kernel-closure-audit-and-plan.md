...
bounded K3-B, K3-C, and K3-D have closed the minimum Relations, Analysis, and
endpoint-projection seams, while integrated consumer closure remains open.
The central factorization has not been contradicted.
...
and bounded K3-D endpoint-projection closure on 2026-08-28. K3-E is current,
and the kernel is not frozen. The R2 protocol-witness track
...
| OIR boundary | Bounded K3-D projection seam closed; full OIR remains inactive | PIR source view and static contract; OIR target semantics, local admission, and exact source-relative projection | The 70-case K3-D instrument independently derives the 11-field source graph, candidate OIR, static contract, exact field-mismatch partition, and support frontier. Local validity remains distinct from source-relative correctness. Exact target-only pair normalization/duality, dynamic traces, final grammar, general execution/refinement, protocol-family support, and Stage 4B remain open. |
...
at bounded standalone resolution. K3-B through K3-D have closed bounded
Relations, Analysis, and endpoint-projection contract/refusal slices without
inventing parallel Protocol meaning or forcing changes upstream. The remaining
immediate consumer risk is the joined K3-E audit. The current
...
owner. K1 closes group 1 at standalone Foundation scope and K2 closes groups
2--4 at standalone PIR scope. Before semantic-kernel freeze, K3--K5 must test
their integration and close the remaining groups:
...
K2 selected the finite same-Core Fresh/FS model specified by
[`pir/interactive-core.md`](../../pir/interactive-core.md) and
[`pir/fiat-shamir.md`](../../pir/fiat-shamir.md). It closes causal strategy
...
K3-B through K3-D are bounded-complete at their stated resolutions; K3-E
integrated closure remains open. K3-C fixes
...
premises; the finite instrument proves none of them. K2 was not reopened.

K3-D then selected one literal PIR-owned purpose view, a minimum
target-semantic OIR body and source-independent local admission, exact
11-field graph equality, and a derived static contract. Its bounded profile
supports P01 FS Verifier and Plan-specialized Prover endpoints only; Fresh,
generic Prover, Oracle, and every module-effect case are typed Unsupported
without a partial target. The source-view ID binds the proposition; whole
source identities bind the validation request/capability, not `OirId`.
Reachable Plan meaning enters the Prover OIR while dead/export material and
concrete suppliers do not. Dynamic endpoint traces and runtime outcomes are
deferred to Stage 4B. The finite instrument is a falsifier, not a refinement or
security proof. K1, K2, and K3-B did not reopen; K3-E integrated review is now
the next gate.
...
appraisal are genuinely downstream. Bounded K3-B through K3-D have closed the
minimum Relations, FS-facing Analysis, and source-relative endpoint-projection
pressure without reopening K1 or K2. K3-C has not supplied
the missing AFK family proof authority, and K3-D has not selected the complete
OIR language. K3-E must later test the integrated no-backflow claim before the
kernel can freeze.
