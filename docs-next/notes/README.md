...
| [Semantic Revalidation and Redesign Cycle](semantic-revalidation-and-redesign/README.md) | Active cycle: K0 integrated-closure reconciliation and the bounded K1 executable-foundations package are complete; K2 Protocol/FS closure is next. R2 remains an evidence track, `P01` Schnorr/Sigma is retained at T3 after final cold rechecks, and `P02` is the next witness anchor but cannot close its portfolio gate before K1--K3 | Exact affected owners under `project/`, `foundation/`, `pir/`, `relations/`, `analysis/`, and `compiler/`; the minimum PIR-to-OIR read seam is active kernel pressure, while full OIR/Realization design remains inactive until separately authorized |
