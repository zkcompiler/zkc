...
| [Semantic Revalidation and Redesign Cycle](semantic-revalidation-and-redesign/README.md) | Active cycle: K0--K2 are bounded-complete after a narrow K2 exact-body reclosure; K3-A consumer-boundary reconstruction is complete and K3-B dependent-surface and Relations co-design is next. R2 remains an evidence track, `P01` is retained at T3, and `P02` cannot close its portfolio gate before K3 | Exact affected owners under `project/`, `foundation/`, `pir/`, `relations/`, `analysis/`, `compiler/`, and the minimum `oir/` seam; full OIR/Realization design remains inactive until separately authorized |
