...
| Check logical-Oracle source Core to committed Core construction | `pir/` | Independently admit both Cores and the exact construction profile; rederive total publication, coin, query, answer/opening, check, outcome, and public-environment maps before minting profile-bound structural authority. The result establishes no commitment property or theorem. |
| Form Fresh and Fiat--Shamir Protocols over the committed Core | `pir/` | Both Protocols retain the exact committed `CoreId`; the Fiat--Shamir arm additionally requires its admitted transcript construction and checked same-Core relation. Neither arm establishes coin-law or security correspondence. |
...
| Causally used initial logical Oracle to confidential Relations grounding | PIR issues the purpose-bound view; `relations/` owns the comparison | Require the identical live causal generation, initial-supply, disclosure-policy, and relation-secret authorities; no public run selector, replay, raw trace, or secret-derived identifier substitutes, and the result retains no carrier |
...
| Checked Oracle-commitment construction to property applicability or transport | `analysis/` | Structural construction authority is only an input. Analysis must additionally bind the exact source and target experiments, commitment assumptions, strategy and coin laws, source theorem and truth authority, resources, and losses; absence yields no property result. |
