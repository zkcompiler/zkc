...
An evidence record owns only its exact observation. It states:
...
Examples in the current hypothesis include PIR-to-OIR projection under
`endpoints/`, admitted-PIR property projection under `judgments/`, and
judgment use in selection constraints under `compiler/`.
...
- evidence versus status: evidence owns the bounded observation, while status
  owns the public capability claim and must narrow it if support is inadequate.
...
Structural admission, relation correspondence, post-seal soundness,
...
- sealing does not establish relation satisfaction or cryptographic security;
