...
law. The [independent service consumer](../zkc-tools/examples/vector-service/main.rs)
exercises both boundaries, including a two-call operation and bad intermediate
replies. See the [library guide](../../docs/compiler/libraries.md).
...
[Architecture, capacity limits and evidence](../../docs/compiler/table-execution.md)
...
See [stateful admission](../../docs/compiler/phase-admission.md#stateful-table-admission).

## Resumable attempts

`attempt::Controller` applies the finite-prefix `iteration` controller to
`Decision::Retry` and `Decision::Complete`. The state belongs to the controller
and survives every retry and stop. `Context::append` maintains a bounded private
output buffer; capacity failure poisons the attempt even if its caller ignores
the returned error. A retry discards only that buffer. Fatal stops remain
terminal, fuel can suspend a finite prefix, and the separate deployment attempt
limit exhausts before starting an extra body and records a zero-byte stop event. Completion returns `Prepared`
bytes that have not been published. Publication and arbitrary host callback
effects require their own contracts. This API does not prove a retry probability
or an eventual-success guarantee.

An input-family runner performs admitted role-local ingress during construction.
Once the entry frame is installed, selector failure returns a stopped runner
with retained backend state and an `ingress.<parameter>` origin. Structural input
admission errors still return `LoadError` with backend custody. `ingress_actions`
...
`admission.rs`, `plan.rs` and `execution.rs` implement the finite closed-source
lifecycle. `interactive/admit.rs` admits participants, `interactive/backend.rs`
defines the native boundary, and `interactive/runner.rs` owns resumable execution
and backend custody. `interactive/transport.rs` defines public actions, message
coordinates, errors and value budgets; existing public imports remain under
`zkc_runtime::interactive`. `iteration.rs` supplies the separate finite-prefix
iteration model used by `attempt.rs`.

Backend implementations and host processes belong to `zkc-backends` and
`zkc-tools`, respectively.
