...
/-! Compositional call bounds over compact source, without expanding shared suffixes.
These count interpreted interface calls, not machine time or native allocations.
-/
...
def Region.callBound (operationBound : language.Op → Nat) {Γ ty} :
    Region language Γ ty → Nat
  | .ret _ | .stop _ => 0
  ...
  | .bind body next => body.callBound operationBound + next.callBound operationBound

theorem Region.denote_within (meaning : Interpretation language interface)
    ...
    PIR.Within (region.callBound operationBound) (region.denote meaning env) := by
  ...
  | bind body next bodyIH nextIH =>
    exact PIR.Boundary.within_bind _ _ _ _ (bodyIH env) (fun value => nextIH (env.push value))
