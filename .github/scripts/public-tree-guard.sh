#!/usr/bin/env bash
# The public tree carries the compiler, its specification, its tests and its
# benchmarks. It deliberately does not carry development-repository material:
# research and review records, strategy and planning queues, session or agent
# process logs, and superseded archives. Those live in the development
# repository and reach the public tree only as finished documentation under
# docs/.
#
...
#
# Run it from the repository root: .github/scripts/public-tree-guard.sh
set -euo pipefail
...
.github/scripts/public-tree-guard.sh in the same change.
EOF
  exit 1
fi
