  ...
  void body(const source::Body &instructions, Env env,
    ...
    for (const auto &instruction : instructions) {
      ...
      if (instruction.get<source::Incomplete>()) {
        ...
        auto selected = resolveBinding(binding, physical);
        ...
        if (physical)
          attrs.push_back(named(b, "kernel", binding.implementation));
        // The selected operation spelling is owned by this temporary
        // contract.
        auto *created =
            operation(b, physical ? "plan.kernel" : selected->operation,
    ...
    for (const auto &binding : m.bindings) {
      ...
      operation(b, "pir.operation_binding", {}, {},
                {named(b, "sym_name", binding.name),
                 ...
                 named(b, "arguments", strings(binding.arguments)),
