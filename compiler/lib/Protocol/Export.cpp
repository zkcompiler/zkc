  ...
  Expected<source::Content> run() {
    ...
    for (auto &op : root->getRegion(0).front()) {
      if (isa<R1CSRelationOp, AIRRelationOp>(op)) {
        ...
        declaration.contract = contract.getValue().str();
        declaration.arguments = strings(arguments, "binding-static-identity");
        declaration.implementation = implementation.getValue().str();
...
LogicalResult verifyModule(Operation *root) {
  ...
  if (!v)
    return root->emitOpError() << toString(v.takeError());
  return success();
}
} // namespace zkc::protocol
