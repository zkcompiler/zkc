...
public:
  ...
  source::Names operation(StringRef contract, source::Names inputs = {},
    ...
    if (found == bindings.end()) {
      ...
      binding.application.contract = contract.str();
      if (contract != "control.require")
        binding.application.arguments = {field};
...
lowerMultilinearR1CS(const R1CS &relation, StringRef prefix, Staging staging) {
  ...
  const uint32_t rows =
      PowerOf2Ceil(std::max<size_t>(2, relation.constraints().size()));
  const uint32_t columns =
      PowerOf2Ceil(std::max<uint32_t>(2, relation.columns()));
