...
class Elaborator {
  ...
  std::optional<std::string> instantiate(const Configuration &configuration,
    ...
    source::walk(*function.body, [&](source::Instruction &instruction) {
      ...
      source::OperationBinding binding{{}, bindingName, call.operation, {}, ""};
      for (unsigned t : call.staticArguments)
        binding.arguments.push_back((*identities)[t]);
      ...
      auto installed = protocol::resolveBinding(binding, false);
      if (!installed) {
