...
class CommitmentCore:
    ...
    def identity(self) -> str:
        return semantic_id("r2.probe.commitment-core.v1", self.term())
...
def build_core(construction: CommitmentConstruction) -> CommitmentCore:
    ...
    profile = CommittedValueProfile(
        name="r2.probe.commitment.profile.committed-vector",
...
def admit_core(core: Any) -> CheckResult:
    ...
    if not isinstance(core, CommitmentCore):
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-000", "not a core")
    actions = core.actions
    if not actions or len(actions) > MAX_CORE_ACTIONS:
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-001", "core size is outside the bound")
    if len(set(core.schedule)) != len(core.schedule):
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-002", "core occurrence is duplicated")
    if actions[-1].kind is not ActionKind.RESIDUAL:
        return _result(OutcomeClass.MISMATCH, "source-boundary", "R2-CMT-CORE-003", "core does not end at its declared residual")
    ...
    for action in actions:
        if not isinstance(action.kind, ActionKind) or not isinstance(action.actor, Actor) or not isinstance(action.value_sort, ValueSort):
            return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-004", "core action vocabulary is open")
        if not action.occurrence.startswith(_KIND_PREFIX[action.kind]):
            return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-005", "action occurrence kind prefix differs")
        if action.label != action.occurrence.split(":", 1)[1]:
            return _result(OutcomeClass.MISMATCH, "closed-core", "R2-CMT-CORE-006", "action label differs from its occurrence")
        if action.actor is not _KIND_ACTOR[action.kind]:
            return _result(OutcomeClass.MISMATCH, "closed-core", "R2-CMT-CORE-007", "action actor differs from its kind")
        if isinstance(action.count, bool) or not isinstance(action.count, int) or action.count <= 0:
            return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-008", "action multiplicity is malformed")
        if action.cardinality is not None and (
            ...
            return _result(OutcomeClass.MALFORMED, "closed-core", "R2-CMT-CORE-009", "action domain is malformed")
        ...
        if bound is not None and action.cardinality is not None and action.cardinality > bound:
            return _result(OutcomeClass.UNSUPPORTED, "closed-core", "R2-CMT-CORE-010", "action domain exceeds its canonical codec profile")
        if action.kind is ActionKind.MESSAGE and action.count != 1:
            return _result(OutcomeClass.MISMATCH, "closed-core", "R2-CMT-CORE-011", "a prover message has multiplicity one")
        if action.kind is ActionKind.CHALLENGE and not action.required_influences:
            return _result(OutcomeClass.MISMATCH, "closed-core", "R2-CMT-CORE-012", "a challenge declares what it binds")
    ...
    if len(profiled) != 1:
        return _result(OutcomeClass.MISMATCH, "commitment:publication", "R2-CMT-CORE-013", "exactly one action publishes the commitment")
    if profiled[0].occurrence not in core.profile.opens_at:
        return _result(OutcomeClass.MISMATCH, "commitment:publication", "R2-CMT-CORE-014", "the profile does not open the action that publishes it")
    ...
    if len(openings) != 1 or openings[0].kind is not ActionKind.MESSAGE:
        return _result(OutcomeClass.MISMATCH, "commitment:opening", "R2-CMT-CORE-015", "an opening is carried by exactly one prover message")
    consumers = [
        a for a in actions
        if a.kind is ActionKind.CHECK and a.predicate is PredicateKind.OPENING_AUTHENTICATES
    ]
    if len(consumers) != 1:
        return _result(OutcomeClass.MISMATCH, "commitment:opening", "R2-CMT-CORE-016", "an opening is consumed by exactly one check")
    if core.schedule.index(openings[0].occurrence) > core.schedule.index(consumers[0].occurrence):
        return _result(OutcomeClass.MISMATCH, "commitment:opening", "R2-CMT-CORE-017", "the opening follows the check that consumes it")
    ...
    # profiled, which R2-CMT-CORE-013 above already enforces.

    return CheckResult(
        outcome=OutcomeClass.AFFIRMATIVE,
        boundary="closed-core",
        code="R2-CMT-CORE-100",
