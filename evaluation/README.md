...
| [`native-fri-ior/`](native-fri-ior/README.md) | Native FRI/IOR semantic validation | One finite logical-oracle FRI case executes through checked commitment, work augmentation, Fresh/Fiat--Shamir pairing, public-only replay, Relations grounding, and Analysis-question formation. A separately coded verifier agrees on selected exact positive public-execution facts; the producer separately classifies two authenticated late negatives at distinct fold and terminal refusals. The result supports the central factorization only at this finite scope; it is not exact paper-algorithm correspondence, family coverage, or a security theorem. |
