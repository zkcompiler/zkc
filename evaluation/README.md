...
| [`k2-protocol-fiat-shamir/`](k2-protocol-fiat-shamir/README.md) | K2 bounded contract vectors and Protocol/Fiat--Shamir behavioral instrument | The 57-test gate includes target-exact K1-backed vectors only for `GuardOutcome` `MF`/`MT` bodies and `OracleAnswer` lookup-result absent/present bodies. The remaining results are fixture-exact or behavioral-shape evidence over a simplified carrier: ordered transcript influence, represented public-coin sinks, Fresh/FS pairing, finite oracle activity, sampling, schedule-anchored reduction declarations, and runtime-only Fresh challenge resolution. It does not implement the complete target carrier, causal capabilities, K1 `AlgorithmUse` path, full `PCNode` graph, sparse absent-lookup execution, or first-class reduction semantics, and it establishes no protocol or security theorem. |
| [`k3-dependent-surfaces/`](k3-dependent-surfaces/README.md) | K3-B dependent-surface boundary instrument | The 29/29 gate exercises finite positive and mutation cases for Interface assignment and Statement coverage, Plan admission and realization, scope-gated public-input reads, witness projection and binding, four-role Relations attachment, replay-qualified public run grounding, semantic/validation identity separation, three non-substitutable value-bridge lanes, artifact facts and equations, and a tagged carrier round trip with a literal disposition manifest over the bounded legacy K2 executable shape. It does not implement or round-trip the durable fourteen-field target carrier. It is a K1/K2-backed reference instrument, not an implementation of the durable schemas, an MLIR carrier, relation satisfaction, theorem applicability, quantitative loss, protocol conformance, cryptographic proof, compiler correctness, or production readiness. R1CS and Nova remain structural fixtures, imported verification remains typed unsupported, and no full FRI/BCS, folding, or protocol-family closure is claimed. |
| [`k3-analysis-closure/`](k3-analysis-closure/README.md) | K3-C minimum Analysis boundary instrument | The 100/100 gate separates a family-neutral AFK theorem schema, abstract-family applicability and transport, and exact pointwise specialization to one native member. It exercises exact source/target experiment profiles and support bindings, Definition 10 quantifier order, the selected `q_KS = 1` consequence, one fixed all-member challenge cardinality, an authenticated and evaluated bounded-bitstring oracle-index bound, a twenty-role family/member manifest with independently resolved content endpoints and separately retained semantic-adequacy premises, independently normalized exact four-formula substitutions, raw Schnorr `Y` with K2-derived fixed-setup provenance, the K2 logical-query carrier, signed lower bounds, theorem applicability, and typed loss imports. The all-`n` source capability, family laws and process correspondence, role-map denotation, the classical ROM, and theorem truth remain explicit premises; a finite native result cannot fill the all-`n` source slot. It is a K1/K2/K3-B-backed reference instrument, not a proof of special soundness, knowledge soundness, Fiat--Shamir security, concrete-hash security, protocol conformance, or implementation support. |
| [`k3-oir-projection/`](k3-oir-projection/README.md) | K3-D bounded OIR-projection instrument | The 70/70 gate separately derives an exact eleven-field PIR source graph, independently constructs and locally admits an OIR graph, checks exact source-relative equality and field-level mismatch sets, derives the nonidentity static endpoint contract, exercises graph-only value access and the reachable Plan quotient, and fails closed at the selected Fresh, generic-Prover, Oracle, and module support boundaries. Its only positive profile is base non-Oracle, non-module P01 FS Verifier and Plan-specialized Prover. JSON identities and Python reflection are probe-local surrogates; the endpoint-pair comparison is non-authoritative; dynamic execution, target-only pairing, full OIR syntax, refinement, protocol-family support, and cryptographic claims remain outside K3-D. |
