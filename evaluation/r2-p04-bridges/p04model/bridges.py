...
class Bridge:
    ...
    def identity(self) -> str:
        return semantic_id("r2.p04.bridge.v1", self.term())
...
def admit_bridge(bridge: Any) -> CheckResult:
    ...
    if not isinstance(bridge, Bridge):
        return _result(OutcomeClass.MALFORMED, "bridge", "P04-000", "not a bridge")

    if bridge.lane is Lane.BIJECTION:
        if bridge.image_predicate is not None:
            return _result(
                OutcomeClass.MISMATCH, "bridge:lane", "P04-001",
        ...
        if bridge.collision_relation is not None or bridge.loss_bits is not None:
            return _result(
                OutcomeClass.MISMATCH, "bridge:lane", "P04-002",
                "a bijection that admits collisions or loss is not a bijection",
            )
        return CheckResult(
            outcome=OutcomeClass.AFFIRMATIVE, boundary="bridge:lane", code="P04-100",
    ...
    if bridge.lane is Lane.EMBEDDING:
        if bridge.image_predicate is None:
            return _result(
                OutcomeClass.MISSING_DEPENDENCY, "bridge:lane", "P04-003",
        ...
        if bridge.collision_relation is not None or bridge.loss_bits:
            return _result(
                OutcomeClass.MISMATCH, "bridge:lane", "P04-004",
                "a mapping that loses information is not a lossless embedding",
            )
        return CheckResult(
            outcome=OutcomeClass.AFFIRMATIVE, boundary="bridge:lane", code="P04-101",
...
def price_projection(
    ...
    if bridge.lane is not Lane.PROJECTION:
        return _result(
            OutcomeClass.MISMATCH, "analysis:projection-loss", "P04-010",
