...
class Claim:
    ...
    def identity(self) -> str:
        return semantic_id("r2.probe.logup.claim.v1", self.term())
...
class LogupCore:
    ...
    @property
    def identity(self) -> str:
        return semantic_id("r2.probe.logup.core.v1", self.term())
...
def admit_core(core: Any) -> CheckResult:
    ...
    if not isinstance(core, LogupCore):
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-LOGUP-000", "not a logup core")
    if len(core.schedule) > MAX_ACTIONS or len(core.claims) > MAX_CLAIMS:
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-LOGUP-001", "core exceeds its declared bound")
    ...
    if set(by_role) != set(ROLES):
        return _result(OutcomeClass.MISMATCH, "logup:roles", "R2-LOGUP-002", "the family fills exactly three roles")
    ...
    if set(core.schedule) != set(core.canonical_schedule()):
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-LOGUP-015", "the schedule does not cover exactly the declared occurrences")
    if len(set(core.schedule)) != len(core.schedule):
        return _result(OutcomeClass.MALFORMED, "closed-core", "R2-LOGUP-016", "an occurrence is scheduled twice")
    ...
    if len(names) != len(core.claims):
        return _result(OutcomeClass.MALFORMED, "logup:claims", "R2-LOGUP-009", "claim name is duplicated")
    consumed: dict[str, int] = {name: 0 for name in names}
    for reduction in core.reductions:
        for name in reduction.consumes:
            if name not in names:
                return _result(OutcomeClass.MISMATCH, "logup:claims", "R2-LOGUP-010", "a reduction consumes an undeclared claim")
            consumed[name] += 1
        for name in reduction.produces:
            if name not in names:
                return _result(OutcomeClass.MISMATCH, "logup:claims", "R2-LOGUP-011", "a reduction produces an undeclared claim")
...
def correspondence(core: LogupCore, fixture: FrozenFixture) -> CheckResult:
    ...
    if tuple(sorted(c.role for c in core.columns)) != fixture.roles:
        return _result(OutcomeClass.MISMATCH, "logup:correspondence", "R2-LOGUP-020", "declared roles differ from the fixture")
    if core.challenge.space != fixture.challenge_space:
        return _result(OutcomeClass.MISMATCH, "logup:correspondence", "R2-LOGUP-021", "declared challenge space differs from the fixture")
    return CheckResult(
        outcome=OutcomeClass.AFFIRMATIVE,
